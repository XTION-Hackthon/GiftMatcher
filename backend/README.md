# 智能礼物交换系统接口文档

本项目是一个基于大语言模型（LLM）与混合模因算法（Memetic Algorithm）的礼物交换匹配系统，旨在解决多人场景下的礼物分配与满意度优化问题。

## 接口说明

- **接口地址**: `http://<服务器IP>:8000/match`
- **请求方式**: `POST`
- **数据格式**: `application/json`

### 1. 请求参数

前端需提交包含所有参与者信息的 JSON 数组。

**字段说明**：
- `gift_description`: 用户填写的礼物自然语言描述。
- `quiz_data`: 用户填写的性格问卷数据。

**请求示例**:

```json
[
  {
    "id": "u001",
    "name": "User A",
    "wechat": "user_a_wechat",
    "mbti": "ENTP",
    "gift_description": "微型方舟反应堆模型，虽然不能发电，但在夜里发蓝光，具有赛博朋克风格。",
    "quiz_data": [
      { "question_text": "周末安排", "selected_option": "B. 在实验室搞科研" }
    ]
  },
  {
    "id": "u002",
    "name": "User B",
    "wechat": "user_b_wechat",
    "mbti": "ISFJ",
    "gift_description": "二战复古素描本，附带一套专业炭笔。",
    "quiz_data": [
      { "question_text": "周末安排", "selected_option": "C. 去看画展" }
    ]
  }
]
```

### 2. 返回结果

后端返回一个排序好的链表 (`chain`)，形成闭环结构（如 A -> B -> C -> A）。

**返回示例**:

```json
{
  "chain": [
    {
      "giver_name": "User A",
      "giver_wechat": "user_a_wechat",
      "receiver_name": "User B",
      "receiver_wechat": "user_b_wechat",
      "gift_summary": "方舟反应堆模型",
      "match_reason": "虽然风格迥异，但这能带给接收者从未体验过的科技感，是一次打破常规的尝试。"
    },
    {
      "giver_name": "User B",
      "giver_wechat": "user_b_wechat",
      "receiver_name": "User A",
      "receiver_wechat": "user_a_wechat",
      "gift_summary": "复古素描本",
      "match_reason": "ENTP 内心其实渴望平静，这本素描本能帮助他记录那些稍纵即逝的灵感。"
    }
  ],
  "total_participants": 2
}
```

---

## 快速启动

### 环境依赖

需安装 Python 3.8+ 及以下依赖库：

```bash
pip install fastapi uvicorn openai pydantic python-dotenv
```

### 启动服务

```bash
python run.py
```

启动后：
- 接口地址: `http://localhost:8000/match`
- 在线文档: `http://localhost:8000/docs`

---

## 技术架构详解

系统采用“关注点分离”原则，将非结构化数据处理流程拆解为三个独立阶段，结合启发式算法进行求解。

### 阶段一：数值评分 (Scoring Phase)

**目标**：将非结构化文本转化为高鲁棒性的数值权重矩阵。

- **逻辑**：LLM 仅负责计算匹配度评分 (0-100)，不生成任何文本理由。
- **权重策略**：采用 **50% 性格契合 (MBTI)** + **50% 行为细节 (Quiz/Gift)** 的双核打分机制，确保匹配既有理论依据，又符合生活实际。
- **实现**：
  - 使用 `Pydantic` 进行严格的数据结构校验。
  - 仅输出 `Receiver_ID` 与 `Score` 的对应关系。
  - **优势**：降低 Token 消耗，避免自然语言生成带来的格式解析错误。

### 阶段二：算法求解 (Optimization Phase)

**目标**：在数值矩阵中寻找全局满意度最高的哈密顿回路 (Hamiltonian Cycle)。

- **算法**：自适应混合模因算法 (Adaptive Memetic Algorithm)。
- **核心机制**：
  1.  **OX1 顺序交叉**：从数学层面保证生成的子代为合法的环形排列（无重复、无遗漏）。
  2.  **局部搜索 (2-Opt)**：对生成的子代进行局部优化，通过交换节点位置提升总分。
  3.  **动态逃逸**：当种群陷入局部最优超过一定代数时，引入随机个体以跳出局部极值。

#### 算法参数调优说明

为了平衡计算速度与解的质量，算法采用了以下关键参数配置：

| 参数 | 设定值 | 作用与影响 |
| :--- | :---: | :--- |
| **POP_SIZE (种群大小)** | `40` | **决定搜索广度**。<br>同时维护 40 种不同的送礼链条方案。对于百人以内的规模，该数值能有效保证多样性，避免过早陷入局部最优，同时内存占用极低。 |
| **GENERATIONS (迭代代数)** | `50` | **决定搜索深度**。<br>算法进化的轮数。由于引入了强力的 `local_search` 算子，个体在每一代都能迅速进化，因此 50 代足以收敛至全局最优解，保证接口响应速度。 |
| **ELITISM (精英保留数)** | `5` | **决定算法稳定性**。<br>每代直接“保送”分数最高的 5 个个体到下一代（占比 12.5%）。这既防止了优质解因变异而丢失，又留出了足够的空间给新个体进行探索。 |

### 阶段三：文案生成 (Generation Phase)

**目标**：为确定的匹配关系生成解释文案。

- **逻辑**：算法已确定赠送关系，此阶段仅进行文本生成。
- **策略**：
  - **拒绝套路**：强制禁止使用“虽然...但是...”等转折句式，要求 AI 寻找正向的互补逻辑。
  - **细节挂钩**：文案必须将“礼物的具体特征”与“接收者的具体行为习惯”进行连接，而非仅依赖 MBTI 标签。
- **优势**：实现逻辑解耦，即使文案生成失败，也不影响核心匹配结果的准确性。

---

## 数据流转示例

以 3 位用户（A、B、C）为例：

### 1. 输入处理
接收包含 3 人信息的 JSON 列表。

### 2. 评分矩阵构建
系统分析性格与礼物匹配度，输出数值矩阵（忽略对角线）：

| (收) \ (送) | User A | User B | User C |
| :--- | :---: | :---: | :---: |
| **User A** | 0 | 30 | 95 |
| **User B** | 40 | 0 | 60 |
| **User C** | 90 | 50 | 0 |

### 3. 路径规划
算法寻找最大权重的闭环路径。
- 路径 1: A->B->C->A (总分: 40+50+95 = 185)
- 路径 2: A->C->B->A (总分: 90+60+30 = 180)

**选定路径 1**：`User A -> User B -> User C -> User A`

### 4. 结果生成
系统请求 LLM 为以下配对生成文案：
1. A (科技礼物) -> B (保守性格)
2. B (艺术礼物) -> C (感性性格)
3. C (趣味礼物) -> A (探索性格)

最终输出包含完整文案的 JSON 结果。

---

## 系统稳定性设计

1.  **数据类型校验**
    利用 `Pydantic` 模型在阶段一拦截格式错误的数据。若 LLM 返回格式有误，该条目将被丢弃并填入默认分值 (30分)，防止程序崩溃。

2.  **约束控制**
    - **逻辑层**：矩阵对角线强制置零。
    - **算法层**：OX1 交叉算子保证排列中无重复节点，杜绝自环现象。

3.  **双重兜底策略**
    - **评分兜底**：若 AI 漏评，填入默认分值 (30分)，依赖算法寻找次优解。
    - **文案兜底**：若批量文案生成失败，系统会自动触发**单次定向生成**请求；若再次失败，则使用通用文案替代，确保前端展示始终完整。

---

## CLI 命令行模式（飞书数据读取）

除了 API 接口模式，系统还支持通过命令行直接从飞书多维表格读取数据并运行匹配分析。

### 环境配置

在 `.env` 文件中添加飞书配置（或使用默认值）：

```bash
# 飞书多维表格配置
FEISHU_APP_TOKEN=T2WsbFLR3aNNnlscrQCchqjGn7c
FEISHU_TABLE_ID=tblZPt93lnlPzIM8
FEISHU_TENANT_ACCESS_TOKEN=t-g104cdmaMN6BP5GKQALRBVBVKIHZI36ATWHYO7EK
```

> ⚠️ **注意**: `tenant_access_token` 有效期为 2 小时，过期后需重新获取。

### CLI 命令

```bash
# 仅读取并显示飞书表格数据（不进行匹配分析）
python cli.py fetch

# 读取飞书数据并运行完整的礼物匹配分析（不发送邮件）
python cli.py analyze

# 读取飞书数据、运行匹配分析并在确认后发送邮件通知
python cli.py match

# 显示帮助信息
python cli.py help
```

### 飞书表格字段映射

CLI 会自动将飞书表格字段映射为系统所需的参与者信息：

| 飞书表格字段 | 系统字段 | 说明 |
| :--- | :--- | :--- |
| 选手名 | name | 参与者姓名 |
| 邮箱 | email | 邮箱地址 |
| 微信账号 | wechat | 微信号 |
| MBTI | mbti | MBTI 性格类型 |
| 准备的礼物描述 | gift_description | 礼物描述 |
| 用户选择题的答案 | quiz_data | 问卷答案 |

### 使用流程

1. 前端收集用户信息并存入飞书多维表格
2. 运行 `python cli.py fetch` 确认数据已正确录入
3. 运行 `python cli.py analyze` 执行完整匹配分析
4. 查看终端输出的匹配结果
5. 运行 `python cli.py match` 发送邮件通知给所有参与者

---

## 🆕 新功能

### ✨ 无MBTI处理

系统现在可以智能处理没有填写MBTI的参与者：
- **评分策略调整**: 无MBTI时，100%依赖问卷数据（生活方式匹配）
- **文案生成优化**: 不会提及MBTI，完全基于兴趣爱好和生活习惯
- **兼容性**: 有MBTI和无MBTI的参与者可以混合匹配

### 📧 邮件通知功能

匹配完成后可以自动发送邮件通知：
- **送礼人邮件**: 告知送礼对象、微信、礼物信息和匹配理由
- **收礼人邮件**: 告知送礼人、微信、礼物信息和匹配理由
- **精美模板**: HTML格式邮件，圣诞主题设计
- **批量发送**: 自动发送给所有参与者

#### 邮件配置

在 `.env` 文件中添加邮件服务配置：

```bash
# 邮件服务配置
SMTP_HOST=smtp.qq.com          # SMTP服务器地址
SMTP_PORT=465                  # SMTP端口（SSL）
SMTP_USER=your_email@qq.com    # 发件邮箱
SMTP_PASSWORD=your_auth_code   # 邮箱授权码（不是登录密码！）
SENDER_NAME=圣诞礼物匹配系统    # 发件人显示名称
```

**如何获取邮箱授权码？**
- QQ邮箱: 设置 → 账户 → POP3/SMTP服务 → 生成授权码
- 163邮箱: 设置 → POP3/SMTP/IMAP → 开启服务 → 设置授权密码
- Gmail: 开启两步验证 → 生成应用专用密码

详细配置指南请查看 [TEST_GUIDE.md](./TEST_GUIDE.md)

---

## 🧪 快速测试

### 本地测试（推荐）

不需要飞书数据，使用模拟数据快速测试所有功能：

```bash
cd backend
python test_local.py
```

测试菜单：
- `1` - 测试匹配算法（不发邮件）
- `2` - 测试完整流程（匹配 + 邮件）
- `3` - 发送单封测试邮件
- `4` - 查看测试数据

详细测试指南请查看 [TEST_GUIDE.md](./TEST_GUIDE.md)

---

## API 接口更新

### POST /match
匹配礼物（不发送邮件）

**请求**: 参与者列表
**返回**: 匹配结果

### POST /send-emails
发送匹配结果邮件

**请求**:
```json
{
  "matches": [...],      // 匹配结果列表
  "participants": [...]  // 参与者列表（包含邮箱）
}
```

**返回**:
```json
{
  "success": 10,   // 成功发送数量
  "failed": 0,     // 失败数量
  "skipped": 0,    // 跳过数量（无邮箱）
  "message": "..."
}
```

### POST /match-and-send
匹配并可选发送邮件

**请求**: 参与者列表 + `send_email` 参数
**返回**: 匹配结果